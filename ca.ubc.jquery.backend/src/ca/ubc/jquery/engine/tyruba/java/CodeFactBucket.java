...
import ca.ubc.jquery.tyruba.javadoc.JQueryTagProcessor;

@Feature(names = { "./annotations", "./NONE" })
public abstract class CodeFactBucket extends JQueryResourceParser {
	...
	protected String cuSource = null;

	protected int markerIDCt = 0; // used to generate marker identifier

	protected static Hashtable javadocTagProcessors = new Hashtable();
	...
	/**
	 * A reference to the RuleBaseManager is needed to be able to add
	 * bucketDependencies into the factbase automatically.
	 */
	protected JQueryResourceManager ruleBaseManager = null;
	...
	private String getQNameFromBinding(ITypeBinding binding) {
		...
		String typeQName = binding.getName();
	...
	public Object getRepresentation(IBinding binding) {
		...
		if (binding instanceof ITypeBinding) {
			...
			strRep = getQNameFromBinding((IMethodBinding) binding);
		} else if (binding instanceof IVariableBinding) {
			IVariableBinding varBinding = (IVariableBinding) binding;
			if (varBinding.isField()) {
				strRep = getQNameFromBinding(varBinding.getDeclaringClass()) + "#" + varBinding.getName();
			} else {
		...
		return makeTypeCast(type, strRep);
	 ...
	 */
	public Object getRepresentation(final IJavaElement je) {
	...
	public void assertTypeCast(Object callerRep, String callerName, Object typeRep, String typeName, int start, int length) {
		String srString = callerName + "typecasts to " + typeName;
		Object sr = makeSourceLocation(start, length);

		insertFact("typeCast", callerRep, typeRep, sr);
		insertFact("primLabel", sr, srString);
	}
	...
	public void assertDefaultValue(Object annotAttrib, Object value) {
			...
			for (int i = 0; i < markers.length; i++) {
				if (!isMarkerAdded(markers[i])) {
					...
					if (start >= sourceStart && end <= (sourceStart + sourceLength)) {
						...
						// TODO awjb: Ignore certain types of markers (e.g. ones generated from JQuery)
