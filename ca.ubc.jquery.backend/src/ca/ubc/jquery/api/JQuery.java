...
public abstract class JQuery implements Cloneable, Serializable {
	...
	protected JQuery(String query) {
		...
		// TODO This has to be concurrent because when binding variables, we sometimes 
		// rename them.  The problem is that ConcurrentHashMap is probably not as efficient
		// as it's regular HashMap counterpart... it would be nice not to use this
	...
	public void setString(String query) throws JQueryException {
		...
		for (Iterator it = boundVars.entrySet().iterator(); it.hasNext();) {
			...
			try {
				...
				// ignore bindings that fail and we obtain a list that has all previous
				// variables still bound and omits new/removed variables
			}
		}

		// clear the chosen vars list
		// TODO Is this the right behaviour?  Should we try to maintain the variables 
		// selected that still exist in the query?
		chosenVars.clear();
	}
	...
	public JQueryResultSet execute() throws JQueryException {
		// in case any modifications to the query string happen here
	...
	public List getChosenVars() throws JQueryException {
		...
		if (chosenVars != null && !chosenVars.isEmpty()) {
			...
			// don't auto-select This var
			chosen.remove(JQueryAPI.getThisVar());
		}
		// remove this var if not allowed
