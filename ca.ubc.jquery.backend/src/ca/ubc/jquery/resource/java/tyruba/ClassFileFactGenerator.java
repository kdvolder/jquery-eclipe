...
public class ClassFileFactGenerator implements ClassVisitor {
	...
	public MethodVisitor visitMethod(int access, String name, String desc,
		...
		if (visitMethodBodies)
			return new ClassFileCodeFactGenerator(typeRepresentation, methodRep, methodName, unqualifiedClassName, packageName, bucket);
		else
			return null;
	}
	...
	private Object getRepresentation(Type type) {
		...
		case Type.LONG:
		case Type.DOUBLE:
			// treat primitives the same as in .java files - no int rep for them
		...
		case Type.ARRAY:
			...
			return bucket.makeTypeCast(bucket.type_RefType, bucket.getIntRep(result));
	...
	private Object getTypeRepresentationFromAsmClassName(String asmName) {
		...
		String clsName = getUnqualifiedNameFromAsmClassName(asmName).replace('$', '#');
		bucket.addDependencyOn(getQualifiedNameFromAsmClassName(asmName));
		if (pkgName.equals("")) {
			return bucket.makeTypeCast(bucket.type_RefType, bucket.getIntRep(clsName));
		} else {
			return bucket.makeTypeCast(bucket.type_RefType, bucket.getIntRep(pkgName + "%." + clsName));
		}
	}
	...
	private Object getPackageRepresentationFromAsmClassName(String name) {
		...
		return bucket.makeTypeCast(bucket.type_Package, bucket.getIntRep(packageName));
	}
	...
	private Object getFieldRepresentation(String name) {
		...
		return bucket.makeTypeCast(bucket.type_Field, bucket.getIntRep(typePrefix + "#" + name));
	}

	private Object getArrayRep(RBTerm elementType) {
		return bucket.makeTypeCast(bucket.type_RefType, bucket.getIntRep(getString(elementType) + "[]"));
	}
	...
	private Object getMethodRepresentation(String name, Object[] argumentTypeReps) {
		...
		methodRepString.append(")");
		return bucket.makeTypeCast(bucket.type_Method, bucket.getIntRep(methodRepString.toString()));
	}
	...
	private Object getConstructorRepresentation(Object[] argumentTypeReps) {
		...
		methodRepString.append(")");
		return bucket.makeTypeCast(bucket.type_Constructor, bucket.getIntRep(methodRepString.toString()));
	}

	private Object getInitializerRep(String methodName) {
		String typePrefix = getString(typeRepresentation);
		return bucket.makeTypeCast(bucket.type_Initializer, bucket.getIntRep(typePrefix + "#" + methodName));
	}
	...
	public AnnotationVisitor visitAnnotation(String desc, boolean visible) {
		// TODO: parse annotations in .class files?
		return null;
	}

	@Override
	public void visitOuterClass(String owner, String name, String desc) {
		// TODO: Uuseful info to assert?
	}
